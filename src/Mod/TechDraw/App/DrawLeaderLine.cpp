...
DrawLeaderLine* DrawLeaderLine::makeLeader(DrawViewPart* parent, std::vector<Base::Vector3d> pagePoints, int iStartSymbol, int iEndSymbol)
{
    ...
    leaderFeature->WayPoints.setValues(leaderPoints);
...
//! return scaled and rotated copies (almost qt ready coords? not rez'd, not inverted) of the WayPoints (page position deltas from 1st point)
//! used by QGILL.
