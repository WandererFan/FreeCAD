...
//    Base::Console().Message("TTL::setUiPrimary()\n");
    ...
    ui->dsbWeight->setValue(TechDraw::LineGroup::getDefaultWidth("Thin"));
    ...
    if (obj->isDerivedFrom(TechDraw::DrawLeaderLine::getClassTypeId())) {
        ...
        if (!sceneDeltas.empty()) {
            ...
            for (auto& delta : sceneDeltas) {
                Base::Vector3d deltaInPageCoords = DGU::fromSceneCoords(delta);
...
// should just do this in place.
            ...
            // now to unscaled, unrotated canonical points
            auto temp = m_lineFeat->makeCanonicalPoints(pageDeltas);
...
//                            m_pbTrackerState);
    ...
    if (getCreateMode()) {
        ...
        for (auto& entry : pageDeltas) {
            m_sceneDeltas.push_back(DGU::toSceneCoords(entry));
        }
